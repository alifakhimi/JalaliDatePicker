...
{
    str = String(str)
    for (var i = 0, numbersLen = constants.englishNumbers.length; i < numbersLen; ++i)
        str = str.replace(constants.englishNumbers[i], constants.persianNumbers[i]);
...
function jalaliToGregorian(j_y, j_m, j_d)
{
    ...
    for (var i=0; i < jm; ++i) j_day_no += constants.j_days_in_month[i];
...
function gregorianToJalali(g_y, g_m, g_d)
{
    ...
    for (var i = 0; i < gm; ++i)
        g_day_no += constants.g_days_in_month[i];
    ...
    for (var i = 0; i < 11 && j_day_no >= constants.j_days_in_month[i]; ++i) {
        j_day_no -= constants.j_days_in_month[i];
    }
    var jm = i + 1;
    var jd = j_day_no + 1;
