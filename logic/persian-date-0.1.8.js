...
//            }
   ...
   */
    format: function (inputString) {
        ...
        function replaceFunction(input) {
            var formatToPersian = self.formatNumber();
            switch (input) {
                // AM/PM
            case("a"):
            {
   ...
   */
    unix: function (timestamp) {
        if (timestamp) {
            ...
            var output = str.substring(0, str.length - 3);
        }
   ...
   * @returns {*}
   */
    getFirstWeekDayOfMonth: function (yearInput, monthInput) {
        var year = yearInput ? yearInput : this.year();
        var month = monthInput ? monthInput : this.month();
    ...
    nextMonth: function(input) {
        var pd = new PersianDate(this)
        pd.date(1)
        var m = pd.month() + (input || input === 0 ? input : 1)

        if(m > 12) {
            pd.year(pd.year() + Math.floor(m / 12))
            m = m - 12
        }
    ...
    prevMonth: function(input) {
        var pd = new PersianDate(this)
        pd.date(1)
        var m = pd.month() + (input || input === 0 ? input : -1)
        if(m < 1) {
            pd.year(pd.year() - Math.ceil(-m / 12) - 1)
            m = 12 + m
        }
